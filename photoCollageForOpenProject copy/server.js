...
var ref5 = db.ref("locations");
var ref6 = db.ref("time");
...
ref2.once('value').then((snapshot) => {
  ...
  let time = 5000 + Math.random() * 1000;
  setInterval(() => {
...
//  })
 ...
 ref5.once('value').then((snapshot) => {
  ...
  for(let i = 0; i < keys.length; i++){
  let key = keys[i];
  dataPoint = address[key];
  console.log(dataPoint);
  arrAddress.push(dataPoint);
  }
  console.log(arrAddress)
  socket.emit('arrAddress', arrAddress);
})




 ref6.once('value').then((snapshot) => {
  ...
  dataPoint = time[key];
  console.log(dataPoint);
  timeArr.push(dataPoint);
  // console.log(timeArr);
  }
   socket.emit('timeArr', timeArr);
})
